...
const AdminPanel = () => {
  ...
  const handleUpdateItem = async (id) => {
    const { name, price, description, longdescription, category } = editFormData;
    ...
    const { data, error } = await supabase
      .from('music_items')
      .update({ name, price: Number(price), description, longdescription, category })
      .eq('id', id)
      .select();
            ...
            <div key={item.id} className="card albom">
              {editingItem === item.id ? (
                <>
                  ...
                  <input type="text" name="longdescription" value={editFormData.longdescription} onChange={handleEditChange} />
